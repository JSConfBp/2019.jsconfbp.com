...
const PackageTable  = () => (
    ...
    {packages.map(pkg => (<>
        <h4 className={ styles[`package_${pkg.name.toLowerCase()}_name`] }>{ pkg.name }</h4>
        <h5 className={ styles[`package_${pkg.name.toLowerCase()}_price`] }>{ pkg.price }</h5>
        {contents.map(cont => (
          <>
            <span className={styles[`package_${pkg.name.toLowerCase()}_${cont.field}_label`]}>
              { cont.desc }
            </span>
            <strong className={[
              styles[`package_${pkg.name.toLowerCase()}_${cont.field}_value`],
              ((pkg.content[cont.field]) ? styles[`package_${pkg.name.toLowerCase()}_${cont.field}_included`] : '')
            ].join(' ')}>
  ...
  <>
    <div className={styles.packages}>
      <div className={styles.block_inner}>
        ...
        <h3>Help us make this event <br />a better experience for our attendees</h3>
